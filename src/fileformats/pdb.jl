...
function load_pdb(fname::String, T=Float32)
    ...
    # for pdb assembly file, if there are two models, the second model might be the transformation of the first model to form a dimer.
    len = length(orig_pdb)
    ...
    for i in 1:length(orig_chains)
        @eval $(Symbol("chain" * string(i))) = getproperty($orig_chains[$i], :id)
    end
    ...
    orig_df = if len == 2
        ...
        if chain1 == @eval $(Symbol("chain" * string(2)))
            if chain1 == "B"
                orig_df2.chainid .= "C"
                @eval $(Symbol("chain" * string(2))) = "C"
            else
                orig_df2.chainid .= "B"
                @eval $(Symbol("chain" * string(2))) = "B"
            end
        end
    ...
    for i in 1:length(orig_chains)
        ...
        fragments = DataFrame(
            ...
            chain_id = @eval $(Symbol("chain", string(i)))
        )
        
        id = @eval $(Symbol("chain", string(i)))
        push!(chains, PDBChain(id, fragments))
    end
