# calculate interface area and interface residues between two biological units(constute of peptide chains)
...
The buried surface area (BSA) type between two biological units in a biomacromolecule.
It has two fields: "area" and "residues".
"area" shows the total buried surface area of the interface.
"residues" is a DataFrame shows the buried surface area of each residue in the system.
...
function count_points(atom::DataFrameRow, points::Vector{SVector{3,Float64}}, neighbours::DataFrame, n::Vector{Base.Threads.Atomic{Int64}}, probe_radius::Float64, i::Int64)
        ...
        @threads for point in points
                ...
                coordinate = (point * (atom[:radii] + probe_radius)) + atom[:r]
                ...
                for neighbour in eachrow(neighbours)
                        ...
                        if (euclidean(coordinate, neighbour[:r]) - neighbour[:radii] - probe_radius) < 0.001
...
"""
        interface(atoms::DataFrame, a::String, b::String, num_points::Int = 92, probe_radius::Number = 1.4, details::Bool=false) 

Calculate the buried surface area between two biological units (a and b, chain ids) in the same DataFrame (resolution: atom), ligands are included, but water molecules (solvent) are excluded.
It returns a Interface object with two fields, "area" and "residues".

"area" shows the total solvent-accessible surface area.
"residues" is a DataFrame shows the buried surface area of each residue in the system.
...
function interface(atoms::DataFrame, a::String, b::String, num_points::Int = 92, probe_radius::Number = 1.4, details::Bool=false) 
        ...
        hgo = [hash_grid_origin for i in 1:nrow(atoms)]
        gz = [grid_size for i in 1:nrow(atoms)]
...
"""
        interface(m::PDBMolecule, a::String, b::String, num_points::Int = 92, probe_radius::Number = 1.4, details::Bool=false)
Calculate the buried surface area between two biological units (a and b, chain ids) in the same PDBMolecule, ligands are included, but water molecules (solvent) are excluded.
It returns a Interface object with two fields, "area" and "residues".
...
"""
interface(m::PDBMolecule, a::String, b::String, num_points::Int = 92, probe_radius::Number = 1.4, details::Bool=false) = interface(m.atoms, a,b,num_points,probe_radius,details)

# if chain ids are not given,the BSA of the interface between the first two chains are calculated
